...
void remove_frame_list(Display *display, struct Workspace_list* workspaces, int index) {
  ...
  free(frames->list);
  if(frames->workspace_name != NULL) XFree(frames->workspace_name);
...
/*    printf("Error, could not load program name for window %lu\n", window);  
    ...
    make_default_program_name(display, window, "Default");
...
void change_to_workspace(Display *display, struct Workspace_list *workspaces, int index,  Window sinking_seperator, Window tiling_seperator) {
  if(index < workspaces->used) {
    ...
    XFlush(display);
    for(int i = 0; i < frames->used; i++) stack_frame(display, &frames->list[i], sinking_seperator, tiling_seperator);
  }
  else {
