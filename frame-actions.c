...
, int position, int size, const struct Workarea *workarea, struct Themes *themes) {
  ...
  while(True) {
    ...
    for(; i < frames->used; i++) {
      ...
      if(
        ...
        *p == position //find adjacent for enlarging (indirect enlarge)
        &&  *p + *s == *fp + *fs
        &&  *fp <= *p + *s
        ...
        *p > position //find adjacent for enlarging
        &&  *p == *fp
        &&  size_change > 0 //enlarging
        ...
        *p + *s + shrink_margin > *fp //find adjacent for shrinking
        &&  *p + *s <= *fp
        &&  size_change < 0 //shrinking
        &&  *p == position
      ) {
        ...
        position + size  > *fp //find overlapped for enlarging
        &&  position < *fp
        &&  size_change > 0 //enlarging
        &&  *p == position
      ) {
        ...
        *p < *fp + *fs + shrink_margin //find adjacent for shrinking
        &&  *p > *fp
        &&  size_change < 0 //shrinking
        &&  *p < position
      ) {
        ...
        position < *fp + *fs //find overlapped for enlarging
      ...
      if(
        *fs - overlap >= *fmin_size
        ...
        } else if (size_change > 0  &&  position < *p) {
          new_position++;
        }
  ...
  *s = size;
  ...
  resize_frame(display, frames->list[index], workarea, themes);
  return;
}
...
**/
void
maximize_frame (Display *display, struct Workspace *frames, int clicked_frame, const char direction, const struct Workarea *workarea, struct Themes *themes) {
